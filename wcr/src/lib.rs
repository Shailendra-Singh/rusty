...
pub fn get_args() -> MyResult<Config> {
    let matches = App::new("wcr")
        .version("0.1.0")
        .author("Shailendra Singh <gitlab.ammonia290@passinbox.com>")
        .about("Word Count - Rust")
        .arg(
            Arg::with_name("files")
                .value_name("FILE")
                .help("Input file(s)")
    ...
    for filename in &config.files {
        match open(filename) {
            ...
            Ok(file) => {
                ...
                lines_total += result.num_lines;
                words_total += result.num_words;
                bytes_total += result.num_bytes;
                chars_total += result.num_chars;
