...
async fn send_command(mut to_server: net::TcpStream) -> ChatResult<()> {
    println!(
        "Commands:\n\
        join GROUP\n\
        post GROUP MESSAGE...\n\
        Type Control-D (on *nix) or Control-Z (on Windows) to close the connection."
    );
...
fn main() -> ChatResult<()> {
    ...
    task::block_on(async {
        ...
        let to_server = send_command(socket.clone());
